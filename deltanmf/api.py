...
def run_twostage_deltanmf(
    ...
    BASE_SEED = 1337):
    if use_fm is None:
        use_fm = bool(stage1_rel_alpha > 0 or stage2_rel_alpha > 0)
    ...
    if use_fm:
        ...
        X_ntc, X_spec, aligned_gene_names = preprocessing.load_data_twostage_no_se(
            X_control, X_case, gene_names,
            min_cells=MIN_CELLS,
            additional_genes_to_remove=remove_genes_list,
            verbose=False
        )
    ...
    if is_ddp or stage1_use_minibatch_ntc:
        ...
        ntc_kwargs = dict(
            ...
            fm_target_ratio=(stage1_rel_alpha if use_fm else None),
            fm_apply_late=bool(use_fm), fm_last_iters=FM_LAST_ITERS,
